...
## New tools (custom fork additions)

This fork adds three slash commands and a Chinese-locale layer on top of
upstream PR-Agent. They are enabled out of the box; no extra configuration
is required beyond the existing `config.response_language` knob.

- `/apply_suggestion <id|all>` — apply a single code suggestion (commits the
  improved snippet to the MR's source branch) or all pending suggestions.
- `/ignore_suggestion <id|all>` — dismiss a single suggestion or all pending
  suggestions. The suggestion is kept in the audit log with a timestamp and
  the actor's username.
- `/suggestion_status` — print a status summary (total / applied / ignored /
  pending).

Inline-reply shortcut: reply `/apply` or `/ignore` (or `/dismiss`) on the
discussion thread that wraps a single inline suggestion — the bot resolves
the suggestion and records the actor. The webhook detects the parent
discussion's `ai-sugg-id` HTML comment and routes the reply to the right
record.

### Block-merge behaviour

After `/improve` finishes, the bot refreshes the MR title to `Draft: <title>`
(equivalent to the GitLab `draft: true` state) as long as at least one
suggestion is still `pending`. When every suggestion has been resolved
(applied, ignored, or accepted by the user), the title prefix is stripped
and the merge button is re-enabled. The state file lives in
`$PR_AGENT_DATA_DIR/suggestions/<project>-mr-<iid>.json` (default
`/tmp/pr_agent_data/...`).

### Chinese locale

`pr_agent/algo/i18n.py` provides a small `t(key, default, **fmt)` helper
backed by an in-memory string table. When `config.response_language` starts
with `zh` (e.g. `zh-CN`), the table is consulted; otherwise `t()` falls
back to the supplied default (English). All hardcoded strings in
`pr_code_suggestions.generate_summarized_suggestions`, the `push_inline_code_suggestions`
inline body, and the action-tool replies are translated. The free-form
LLM-generated text (`suggestion_content`, `score_why`, `one_sentence_summary`)
is already routed through the upstream `response_language` mechanism
in `pr_agent/agent/pr_agent.py`.

### Suggested `pr_agent/settings/configuration.toml` overrides for this fork

```toml
[config]
response_language = "zh-CN"        # turn on Chinese UI + LLM responses
is_auto_command = true            # so /improve messages don't include the "explore" preamble

[pr_code_suggestions]
focus_only_on_problems = false    # allow style/optimization suggestions, not just bugs
enable_help_text = true           # show the inline help/usage guide in the MR comment
