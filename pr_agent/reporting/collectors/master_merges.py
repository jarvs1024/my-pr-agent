...
要求 (markdown 格式, 中文):
- **禁止使用 markdown # 标题** (DingTalk 面板里会被渲染成超大字号, 严重占用空间)。改用 **粗体** 替代,例如 `**本周变更概览**`。
- 一段概述 (1-3 句): 本周变更主题、涉及的主要模块、整体方向,放在 `**概述**` 段落里。
- 分类要点 (bullet),按需给出,空类别省略;每条类别名也用 **粗体**,不要用 `### 新增` 这种:
  - **新增**: 新文件 / 新模块 / 新接口
  - **修改**: 既有功能调整
  - **删除**: 移除的文件 / 代码 / 逻辑
  - **测试**: 测试相关变更
...
直接输出 markdown,不要加开场白,不要在任何行首使用 `#` 字符。
""")
